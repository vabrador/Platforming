...
public class PlayerCameraController : MonoBehaviour {
	...
	void Awake() {
		...
		player.OnReferenceFrameTranslated += onPlayerReferenceFrameTranslated;
	}
	...
	void Update() {
		...
		this.transform.RotateAround(player.transform.position, Vector3.down, _manualTwistVelocity * Time.deltaTime);
	...
	private float getRelativeAltitudeToTarget() {
		return Vector3.Dot(altitudeDirection, (this.transform.position - player.transform.position));
	}

	private float getRelativeForwardOffsetToTarget() {
		return Vector3.Dot(forwardDirection, (this.transform.position - player.transform.position));
	}
